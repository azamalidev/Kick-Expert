...
export async function POST(req: Request) {
  console.log('📧 Referral confirmed email API called');
  ...
  try {
    const body = await req.json() as ReferralEmailRequest;
    console.log('📦 Referral email request payload:', {
    ...
    if (!referrerId || !referredUserId || !xpAwarded || totalReferrals === undefined) {
      console.error('❌ Missing required fields:', { referrerId, referredUserId, xpAwarded, totalReferrals });
    ...
    console.log('📝 Generating referral confirmation email template...');
    const emailTemplate = transactionalTemplates.referralConfirmed({
      name: full_name,
      referrerName: full_name,
      xpAwarded,
      totalReferrals,
      nextMilestone,
      referralCredits,
    ...
    console.log('📤 Sending referral confirmation email via Brevo to:', email);
    ...
    console.log('✅ Referral confirmation email sent successfully! MessageId:', responseData.messageId);

    return NextResponse.json({
      success: true,
      messageId: responseData.messageId,
      message: 'Referral confirmation email sent successfully',
    });

  } catch (error) {
    console.error('❌ Error sending referral confirmation email:', error);
    return NextResponse.json({
